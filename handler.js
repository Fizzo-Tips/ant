...
export async function participantsUpdate({ id, participants, action }) { 
 ...
 const anu = {
  ...
  "title": `Kiro`,
  "description": 'Member baru jangan sok kerasd',
  "currencyCode": "US",
  "priceAmount1000": "100",
  "retailerId": wm,
 ...
             if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat Datang Todd, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            (chat.sBye || this.bye || conn.bye || 'Selamat tinggal, @user!')).replace('@user', `${this.getName(user)}`)
                        this.sendButtonImg(id, pp, wm, text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.jsdkllasd' : 'Wellcome', anu, { contextInfo: { externalAdReply: { showAdAttribution: true,
mediaUrl: sgc,
mediaType: 2, 
description: sgc,
title: "KIBOT OFFICIAL",
body: wm,
thumbnail: await(await fetch(thumb)).buffer(),  // fs.readFileSync('./thumbnail.jpg'),
sourceUrl: sgc
}}
})
                }
...
Fitur ini di *disable* !`
    ...
    // if (msg) return conn.sendFile(m.chat, `${imgr + 'Accses Ditolak KiBOT'}`, 'denied.jpg', msg, m, { contextInfo: { externalAdReply: {title: global.wm, body: '404 Access denied ✘', sourceUrl: global.snh, thumbnail: fs.readFileSync('./thumbnail.jpg') }}})
